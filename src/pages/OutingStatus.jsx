...
const OutingStatus = () => {
    ...
    const fetchMonthlyOutings = async () => {
      try {
        ...
        const response = await fetch(
          "https://outing-backend-83sg.onrender.com/outing/latestOuting",
          {
            headers: {
  ...
  const cancelOuting = async (outingId) => {
    try {
      const token = localStorage.getItem("studentLoginToken");
      const response = await fetch(
        `https://outing-backend-83sg.onrender.com/outing/cancel/${outingId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `${token}`,
      ...
      if (response.ok) {
        ...
        setMonthlyOutings(
          monthlyOutings.filter((outing) => outing._id !== outingId)
        ); // Remove the canceled outing
      } else {
    ...
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-r from-blue-100 to-gray-100 py-8">
      ...
      {loading ? (
        ...
        monthlyOutings.map((outing) => (
          <div
            key={outing._id}
            className="w-full max-w-lg bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-lg p-8 border border-gray-200 transition-transform transform hover:scale-105 mb-6"
          >
            <h2 className="text-2xl font-semibold text-gray-800 mb-4 text-center">
              Outing on {formatDate(outing.date)}
            </h2>

            <p className="text-lg font-semibold text-blue-800 mb-2">
              <strong>Date:</strong> {formatDate(outing.date)}
            </p>
            <p className="text-md text-gray-700 mb-1">
              <strong>Reason:</strong> {outing.reason}
            </p>
            <p
              className={`mt-2 text-md font-semibold ${getStatusStyle(
                outing.outingStatus
              )}`}
            >
              Status:{" "}
              {outing.outingStatus.charAt(0).toUpperCase() +
                outing.outingStatus.slice(1)}
            </p>
        ...
        <div className="text-center">
          <p className="mt-12 text-gray-600 text-center text-lg">
            No recent outings found for this month.
          </p>
          <button
            onClick={() => navigate("/welcome")}
