...
const CaretakerWelcome = () => {
  ...
  useEffect(() => {
    const fetchOutings = async () => {
      try {
        ...
        const response = await fetch(
          "https://outing-backend-83sg.onrender.com/outing/all-outings",
          {
            headers: {
    ...
    const getCareTakerName = async () => {
      try {
        const token = localStorage.getItem("caretakerLoginToken");
        const response = await fetch(
          "https://outing-backend-83sg.onrender.com/careTaker/singleCareTaker",
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `${token}`,
  ...
  const fetchStudentIds = async (outings) => {
    try {
      ...
      for (const outing of outings) {
        const response = await fetch(
          `https://outing-backend-83sg.onrender.com/student/${outing.studentId._id}`,
          {
            headers: {
              Authorization: `${token}`,
            },
  ...
  const updateStats = (outings) => {
    ...
    const pending = outings.filter(
      (outing) => outing.outingStatus === "pending"
    ).length;
    const accepted = outings.filter(
      (outing) => outing.outingStatus === "accepted"
    ).length;
    const rejected = outings.filter(
      (outing) => outing.outingStatus === "rejected"
    ).length;
  ...
  const updateOutingStatus = async (outingId, status) => {
    try {
      const token = localStorage.getItem("caretakerLoginToken");
      const response = await fetch(
        `https://outing-backend-83sg.onrender.com/outing/${outingId}/update`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const filteredOutings = outings.filter((outing) => {
    const matchesStatus =
      selectedStatus === "all" || outing.outingStatus === selectedStatus;
    const matchesName = outing.studentName
      .toLowerCase()
      .includes(searchName.toLowerCase());
    const matchesId = studentIds[outing.studentId._id]?.startsWith(searchId);
    const matchesYear =
      searchYear === "" || outing.year.toString() === searchYear;
    const matchesDate =
      searchDate === "" ||
      format(new Date(outing.date), "yyyy-MM-dd") === searchDate;

    return (
      matchesStatus && matchesName && matchesId && matchesYear && matchesDate
    );
  });
      ...
      <main className="flex-1 p-8">
        ...
        <div className="mt-10">
          <h3 className="text-2xl font-bold text-gray-700">
            {selectedStatus.toUpperCase()} Outing Requests
          </h3>
                ...
                {filteredOutings.map((outing, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        onClick={() =>
                          updateOutingStatus(outing._id, "accepted")
                        }
                        className="px-4 py-1 mr-2 rounded-lg bg-green-500 text-white font-semibold hover:bg-green-600"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() =>
                          updateOutingStatus(outing._id, "rejected")
                        }
